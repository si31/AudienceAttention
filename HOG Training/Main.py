...
def collectImages():
	...
	num_pos = 11
	num_neg = 59
	for i in range(1, math.floor((num_pos/4)*3)):
		#load image
		imageWrongSize = cv2.imread('Positives/' + str(i) + '.jpg')
		#images 
		imagesPosTrain.append(imageWrongSize)
	for i in range(math.floor((num_pos/4)*3), num_pos+1):
		#load image
		imageWrongSize = cv2.imread('Positives/' + str(i) + '.jpg')
		#images 
		imagesPosTest.append(imageWrongSize)
	for i in range(1, math.floor(num_neg/2)):
		#load image
		imageWrongSize = cv2.imread('Negatives/img' + str(i) + '.jpg')
		#images 
		imagesNegTrain.append(imageWrongSize)
	for i in range(math.floor(num_neg/2), num_neg+1):
		#load image
		imageWrongSize = cv2.imread('Negatives/img' + str(i) + '.jpg')
		#images 
		imagesNegTest.append(imageWrongSize)
...
def draw_hog(img, hog):
	...
	maxv = np.max(hog)
	for x in range(0, vertCells):
		for y in range(0, horiCells):
			...
			for x1,y1,gv in fdraw:
				cv2.line(img, (px-x1,py+y1), (px+x1,py-y1), (gv, gv, gv), 1, 8)


def main():
	print('Start...')
	
	img = cv2.imread('Positives_occulsion/3.png')
	img = cv2.resize(img, (width,height), interpolation=cv2.INTER_LINEAR);
